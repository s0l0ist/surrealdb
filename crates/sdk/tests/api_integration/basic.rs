...
pub async fn signup_record(new_db: impl CreateDb) {
	let config = Config::new();
	let (permit, db) = new_db.create_db(config).await;
	let namespace = ULID::now().encode();
	let database = ULID::now().encode();
...
pub async fn signin_ns(new_db: impl CreateDb) {
	...
	let (permit, db) = new_db.create_db(config).await;
	let namespace = ULID::now().encode();
	db.use_ns(&namespace).use_db(ULID::now().encode()).await.unwrap();
	let user = ULID::now().encode();
